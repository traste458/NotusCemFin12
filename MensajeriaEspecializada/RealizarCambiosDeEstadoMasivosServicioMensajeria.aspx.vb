...
Public Class RealizarCambiosDeEstadoMasivosServicioMensajeria
...
#Region "Eventos"
        ...
        Try
            ...
            If Not IsPostBack Or Not IsCallback Then
                With miEncabezado
                    ...
                    .setTitle("Realizar Cambios De Estado Masivos Servicio Mensajería")
                End With
            End If
    ...
    Protected Sub btnExportar_Click(sender As Object, e As EventArgs) Handles btnExportar.Click
        gveExportadorErrores.WriteXlsxToResponse("ErroresPlantillaCambioMarcacionSerialesTipoSims")
    End Sub
    ...
    Protected Sub ButtonVerEjemplo_Click(sender As Object, e As EventArgs) Handles ButtonVerEjemplo.Click
        Try
            Dim filename As String = Server.MapPath("~/MensajeriaEspecializada/Plantillas/PlantillaCambiosDeEstadoMasivosServicioMensajeria.xlsx")
            If filename <> "" Then
...
#Region "Métodos Privados"
    Private Sub upArchivo_FileUploadComplete(sender As Object, e As DevExpress.Web.FileUploadCompleteEventArgs) Handles upArchivo.FileUploadComplete
        Try
            ...
            If upArchivo.HasFile Then
                ...
                Dim nombreEquipo As String = System.Net.Dns.GetHostName.ToString()
                Dim columnaEquipo As DataColumn
                ...
                If numColumnas = 9 Then
                    ...
                    For Each row As DataRow In dtUsuarioEjecutor.Rows
                        If Not IsNumeric(row("Radicado")) Then
                            RegError(fil, "El radicado tiene caracteres no validos o no cumple con la longitud requerida: " & row("Radicado"), row("Radicado"))
                        End If

                        If Not IsDBNull(row("FechaAgenda")) Then
                            If Not IsDate(row("FechaAgenda")) Then
                                RegError(fil, "La Fecha de Agenda no es una fecha valida por favor verifique: " & row("FechaAgenda"), row("Radicado"))
                            End If
                        End If
                        'FechaCierre
                        If Not IsDBNull(row("FechaCierre")) Then
                            If Not IsDate(row("FechaCierre")) Then
                                RegError(fil, "La Fecha de Fecha Entrega/Fecha Cierre no es una fecha valida por favor verifique: " & row("FechaConfirmacion"), row("Radicado"))
                            End If
                        End If
                        If Not IsDBNull(row("FechaConfirmacion")) Then
                            If Not IsDate(row("FechaConfirmacion")) Then
                                RegError(fil, "La Fecha de Confirmacion no es una fecha valida por favor verifique: " & row("FechaConfirmacion"), row("Radicado"))
                            End If
                        End If
                        If IsDBNull(row("EstadoActual")) Then
                            RegError(fil, "El Estado Actual no puede estar vacio: " & row("EstadoActual"), row("Radicado"))
                        End If
                        If IsDBNull(row("NuevoEstado")) Then
                            RegError(fil, "El Nuevo Estado no puede estar vacio: " & row("NuevoEstado"), row("Radicado"))
                        End If
                        If Not IsDBNull(row("idJornada")) Then
                            If (row("idJornada") <> "1" And row("idJornada") <> "2") Then
                                RegError(fil, "El valor de la jornada no es valido Valores permitido 1=AM o 2= PM Valor Archivo:  " & row("idJornada"), row("Radicado"))
                            End If
                        End If
                        row("Fila") = fil
                        fil = fil + 1
                    Next
                    If Session("dtResultado") Is Nothing Then
                        columnaEquipo = New DataColumn("nombreEquipo", GetType(System.String))
                        columnaEquipo.DefaultValue = nombreEquipo
                        dtUsuarioEjecutor.Columns.Add(columnaEquipo)

                        dtUsuarioEjecutor.Columns.Add(New DataColumn("idUsuario", GetType(System.Int64), idUsuario))
                        dtUsuarioEjecutor.AcceptChanges()
                        ...
                        With objCambioEstado
                            ...
                            .IdUsuario = idUsuario
                            Session("dtResultado") = .RegistrarCambioEstadoMasivo(dtUsuarioEjecutor, resultado)
                ...
                ElseIf numColumnas < 9 Then
                    ...
                    CType(sender, ASPxUploadControl).JSProperties("cpResultadoProceso") = 0
                Else
                    RegError(" ", "No se puede procesar el archivo, ya que contiene mas columnas que las esperadas. Por favor verifique", "")
                    CType(sender, ASPxUploadControl).JSProperties("cpResultadoProceso") = 0
    ...
    Private Function CrearEstructuraDeTabla() As DataTable
        ...
        With dtAux.Columns
            ...
            dtAux.Columns.Add(New DataColumn("EstadoActual", GetType(String)))
            dtAux.Columns.Add(New DataColumn("NuevoEstado", GetType(String)))
            dtAux.Columns.Add(New DataColumn("FechaAgenda", GetType(String)))
            dtAux.Columns.Add(New DataColumn("FechaConfirmacion", GetType(String)))
            dtAux.Columns.Add(New DataColumn("FechaCierre", GetType(String)))
            dtAux.Columns.Add(New DataColumn("idJornada", GetType(String)))
            dtAux.Columns.Add(New DataColumn("Observacion", GetType(String)))
            dtAux.Columns.Add(New DataColumn("Direccion", GetType(String)))
        End With
        Return dtAux
    End Function
