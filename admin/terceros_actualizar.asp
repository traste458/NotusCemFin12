...
idCargo = cdbl(rs("idcargo"))
estadoCargo = 0
sqlAux = "select estado from cargos where idcargo = " & cstr(idCargo)
set rsEstado =  conn.execute(sqlAux)
if not rsEstado.eof then estadoCargo = cdbl(rsEstado("estado"))
rsEstado.close
...
<TABLE BORDER="1">
  <TR>
    ...
    <TD><INPUT TYPE="TEXT" NAME="tercero" SIZE="40" MAXLENGTH="50" value="<%=trim(rs("tercero"))%>"></TD>   
  </TR>  
  <TR>
  ...
  <TR>
    ...
    <TD>
      <SELECT NAME="idcargo">
		<%if cdbl(estadoCargo) <> 1 then%>
        <OPTION VALUE="0"><%=rs("cargo") & " (INACTIVO)"%></OPTION>
		<%else%>
   		  <OPTION VALUE="0">Escoja un Cargo</OPTION>
		<%end if
		   sql = "select idcargo,cargo from cargos where estado = 1 order by cargo "
       set rs2 = conn.execute(sql)
        while not rs2.eof
				  estadoSel = ""
					if cdbl(idCargo) = cdbl(rs2("idcargo")) then 
					  estadoSel = "selected"
					end if
       %>      
          <OPTION VALUE="<%=rs2("idcargo")%>" <%=estadoSel%> ><%=rs2("cargo")%></OPTION>
       <%rs2.movenext 
        wend
			 %>         
      </SELECT>
...
%>         
			...
			<input type="hidden" id = "idCargoActual" name="idCargoActual" value="<%=idcargo%>"   />
			<input type="hidden" id = "idPosActual" name="idPosActual" value="<%=rs("idpos")%>"   />
			<input type="hidden" id = "estadoActual" name="estadoActual" value="<%=est2%>"   />  
    </TD>   
  </TR>
  <TR>
...
%>         
  ...
  <TR>
    ...
    <TD><INPUT TYPE="TEXT" NAME="Email" SIZE="50" MAXLENGTH="50" value="<%=rs("email")%>" ID="Text1"></TD>   
  </TR>  
