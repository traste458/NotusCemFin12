...
Partial Public Class SimCardReporte
...
#Region "Métodos Privados"
    ...
    Private Sub CargarTipoSim()
        Dim dtEstado As New DataTable
        Try
            dtEstado = HerramientasMensajeria.ConsultarTipoSim
            With ddlTipoSim
                .DataSource = dtEstado
                .DataTextField = "nombre"
                ...
                .DataBind()
                If .Items.Count <> 1 Then
                    .Items.Insert(0, New ListItem("Seleccione un Tipo SIM", "0"))
                End If
            End With
        Catch ex As Exception
            epNotificacion.showError("Error al tratar de obtener el listado de tipos de Sim. " & ex.Message)
        End Try
    End Sub
    ...
    Private Function ConsultarReporte() As DataTable
        ...
        With reporte
            ...
            If ddlTipoSim.SelectedValue > 0 Then .TipoSim = ddlTipoSim.SelectedValue
        End With
    ...
    Private Function GenerarArchivoExcel(ByVal dtDatos As DataTable) As String
        Dim rutaPlantilla As String = Server.MapPath("~/ReportesCEM/Plantillas/PlantillaReporteSimCard.xlsx")
        Dim nombreArchivo As String = Server.MapPath("~/archivos_planos/ReporteSimCard" & Session("usxp001") & ".xls")
        ...
        Else
            miHoja = miExcel.Worksheets.Add("Reporte Sim Card")
